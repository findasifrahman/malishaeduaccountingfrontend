...
import { inventoryitemmodels,inventoryiteminterface } from '../../../../models/inventoryitemmodels';
...
export class InventoryItemListComponent implements OnInit {
  ...
  displayedColumns: string[] = ['inventoryitemname','buttons'];
  displayedColumnsName: string[] = ['Inventory Item Name'];
  AllElement: MatTableDataSource<inventoryiteminterface>;

  constructor(private snackBar: MatSnackBar, private iiService: InventoryItemService, public dialog: MatDialog,
  ...
  ngAfterViewInit(): void {
    this.iiService.getAll().subscribe((posts) => {
  ...
  onDelete(id) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result) {
        this.iiService.delete(id).subscribe((posts) => {
          this.AllElement = new MatTableDataSource(posts as any);
          this.AllElement.paginator = this.paginator;
          console.log(posts);
